...
class sb extends groups {
    ...
    public function queryServers($query) {
        ...
        while ($server = $result->fetch(PDO::FETCH_ASSOC)) {
            ...
            $OUTPUT = $srcds_rcon->rcon_command($server['host'], $server['port'], $server['rcon'], $query);
        }
        return true;
    }
    ...
    public function getUserGroup($steam_id) {// code works good
        try {
            $stmt = $this->sdb->prepare("SELECT * FROM " . SB_PREFIX . "_admins WHERE `identity` =?;");
        ...
        $this->userGroup = array('srv_group' => $srv_group, 'group_id' => $group_id, 'aid' => $aid);
